  ...
  if (!welcomeExists) {
    ...
    db.prepare(
      `UPDATE dialogs
       ...
       WHERE id = 1`
    ).run('Witamy w Spool', welcomeBodyPl);
  }
...
const getUserById = (id) =>
  db.prepare('SELECT id, username, display_name, avatar_path, role, created_at, last_login, can_upload FROM users WHERE id = ?').get(id);

const getAllUsers = () =>
  db.prepare('SELECT id, username, display_name, avatar_path, role, created_at, last_login, can_upload FROM users ORDER BY created_at ASC').all();
...
const updateUser = (id, fields) => {
  const allowed = ['display_name', 'role', 'password_hash', 'avatar_path', 'twofa_secret', 'twofa_enabled', 'can_upload'];
  ...
  const rows = db
    .prepare(
      `SELECT v.id, v.filename, v.title, v.original_title, v.description, v.category, v.content_date, v.file_created_at,
              v.duration, v.file_size, v.thumbnail_path, v.video_width, v.video_height, v.view_count, v.scanned_at, v.channel_id,
              CASE WHEN v.channel_id IS NULL THEN (SELECT value FROM settings WHERE key = 'channel_name') ELSE c.name END AS channel_name, 
              CASE WHEN v.channel_id IS NULL THEN (SELECT value FROM settings WHERE key = 'channel_avatar') ELSE c.avatar_path END AS channel_avatar_path
       FROM videos v 
       LEFT JOIN channels c ON c.id = v.channel_id
